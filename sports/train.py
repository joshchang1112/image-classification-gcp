...
def training(args, train_loader, valid_loader, model, optimizer, scheduler, device):
    ...
    for epoch in range(args.epochs):
        ...
        if valid_acc > best_valid_acc:
            ...
            torch.save(model, os.path.join(args.model_dir, 'best_resnet101.pkl'))
...
def main():
    parser = argparse.ArgumentParser(description='Image Classification for Sports')
    ...
    #model.classifier[-1] = torch.nn.Linear(1024, 100)
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max')
    training(args, train_loader, valid_loader, model, optimizer, scheduler, device) 
    
    #model = torch.load(os.path.join(args.model_dir, 'best_mobilenet_v3.pkl'))
    model = torch.load(os.path.join(args.model_dir, 'best_resnet101.pkl'))
